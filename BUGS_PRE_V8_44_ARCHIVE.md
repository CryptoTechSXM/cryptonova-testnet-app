# ARCHIVE — Issue Tracker up to and including V8.43

> **Archived 2026-07-25 at the V8.44 go-live.** Everything below predates V8.44 and is
> CLOSED for tracking purposes. The live tracker restarted clean at `BUGS.md`.
>
> **What V8.44 resolved from this archive (contract-level, shipped 2026-07-25):**
> - **@CryptoJan "T1 graduated on 4 accounts" + Sherwyn "kicked out of T1 after A & B"**
>   (both under *Fix In Progress*) — root cause was `handleCycleOut` receiving escrow=0,
>   so a passive member's MatB crossing reserve could not fund re-entry. V8.44 passes the
>   reserve as escrow, funds each seat escrow-first, and **parks instead of exiting** when
>   still short. Sherwyn's stated expectation was the correct design and is now the behaviour.
> - **Sherwyn "stuck in Matrix B, not advancing" / "tiers not looping"** (2026-07-25, Open)
>   — root cause was frozen-MatB starvation: at pair saturation, re-entries and rescues were
>   diverted to the NEXT pair, so a full MatB never received the entry that rotates it
>   (live proof: MatA rot 254-291 while MatB rot 0 on pairs 2-5). V8.44 returns a pair's own
>   members to their own MatB and removes the "partner full" block, so a full matrix rotates
>   on the next entry. Keeper force-rotate remains only as a backstop.
> - **Stranded crossing reserves** — `deductForUpgrade` now decrements the reserve properly;
>   clean exits release it to withdrawable; `adminReleaseStrandedReserve` recovers drift.
>   V8.43 stranded wallets can be enumerated with `scripts/enumerate_stranded_v843.js`.
> - **Graceful exit** (Mainnet-Prep question; ThanksAndPraises, Maximum_71, Koach100) —
>   implemented as `exitSeat()`: releases the crossing reserve minus a DAO-set penalty
>   (default 20%, menu 0/10/20/30/50%). Earnings are never penalised.
> - **`bulkUpgrade` vs `manualUpgrade` gate mismatch** (side finding in Sherwyn's gas report)
>   — both now share one three-way eligibility rule (cycle done OR in prev MatB OR gate open).
>
> Not carried forward as bugs: the V8.43 owner items at the top (all shipped), the gas-limit
> fixes (shipped to main 2026-07-24), and the wallet-detection fix on bug-report.html.
