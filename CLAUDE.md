...
| V8.44 BUILD — NEXT SESSION | Owner decision 2026-07-25: build FULL V8.44 (all fixes A–I) in a fresh session, test-first. **Build order + kickoff at the TOP of CryptoNite-Smart-Contracts/CryptoNova/V8_44_PLAN.md** — read it first. Headline finding: the V8.43 CYCLING SLOWDOWN root cause is frozen MatB starvation (passive members park at crossing → rescued members overflow to next pair, not their own MatB → MatB never rotates; live proof: MatA rot 254–291 but MatB rot 0 on factory pairs 2–5). Fix = crossing-fund + overflow rework (own members return to own MatB) + factory-ownership/self-heal. Also in scope: pull-based pool (gas), permit/bulk-withdraw/hybrid-upgrade UX, graceful exit, CW advanceEpoch keeper, stranded-reserve recovery, docs/bot sync. NO clean live fix exists — must redeploy. Anchor: "read memory, then build V8.44 from the build order." |
