...
const TIER_ROUTER       = '0xBacE079aDB755Ea42b32310FE2E414CF036dd318'; // V8.52
const CNOVA_TOKEN       = '0x3848800e1d819EE6823E211B60dA623d9eCD7cdA'; // V8.52
const CNOVA_TREASURY    = '0xfE21170e1128153F5209dA47a332C830D5e9dc90'; // V8.52 — floorPrice() lives here
...
## The Matrix System (verified from contract, V8.52)
...
3. When you reach root of <b>MatA</b> and cycle out, you <b>cross into MatB</b>. The crossing costs the HALF the entry fee (V8.52) — exactly your crossing reserve, which pre-paid it when you entered: nothing is drawn from your earnings, and your earned balance travels with you. Starting the next full cycle after MatB is a new full entry from your accumulated funds; if they come up short, you are parked until the gap is covered (see Rescue).
...
## Compensation Plan (verified from contract, V8.52)
...
Epoch advances when the FIRST fires: 1,000,000 CNOVA minted, 1,000 new members, or 180 days elapsed (V8.52 policy — item 42).
...
On the 25TH of every month a distribution can be triggered (calendar-day model, V8.52): half the pool pays out, half rolls over and compounds. Members then have 30 days to claim; unclaimed amounts sweep back into the pool by design. Payouts begin at mainnet (policy).
...
## Upgrades, Re-entry & Rescue (verified from contract, V8.52)
...
- When the newest pair FILLS, the factory spawns the next pair and overflow routes into it automatically (V8.52 — the old fixed 375/381 entry thresholds are GONE from the contracts; think seats remaining, not entry counts).
- <b>"Why is T1.2 (or any new pair) empty?"</b> New pairs deploy when the newest pair is FULL (V8.52 item 33 — no more fixed entry thresholds). A new pair can sit empty until routing reaches it — only then does new activity flow into it automatically. An empty freshly-deployed pair is the system working correctly: the runway is built before the plane needs it. It is NOT stalled, and nobody is "stuck" in it — reassure members this is by design (V8.52 anti-freeze buffer).
...
## Contracts (Base Sepolia — V8.52)
TierRouter: <code>0xBacE079aDB755Ea42b32310FE2E414CF036dd318</code>
MatrixKeeper: <code>0x6fEee431026d1F7F2538FcB96Da145Fe0AC6DEE2</code>
CNOVA Token: <code>0x3848800e1d819EE6823E211B60dA623d9eCD7cdA</code>
  ...
  if (isCommand) {
    ...
    if (cmd === '/tier') {
      await sendReply(BOT_TOKEN, chatId,
        ...
        `• 2.5% instant earn on join · 50% crossing reserve (pre-pays your crossing in full — V8.52)\n` +
