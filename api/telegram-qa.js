...
const TIER_ROUTER       = '0x72DAF5647Dff8E71D0c4ECB768B19E77DcBcba63'; // V8.52 PRIVATE
const CNOVA_TOKEN       = '0x0DB48e7Aa5a972627bFA2f69f518328c053B0832'; // V8.52 PRIVATE
const CNOVA_TREASURY    = '0xe023917172Aa120D707FD6a0f447e6266A47c716'; // V8.52 PRIVATE — floorPrice() lives here
...
## The Matrix System (verified from contract, V8.52 PRIVATE)
...
3. When you reach root of <b>MatA</b> and cycle out, you <b>cross into MatB</b>. The crossing costs the HALF the entry fee (V8.52 PRIVATE) — exactly your crossing reserve, which pre-paid it when you entered: nothing is drawn from your earnings, and your earned balance travels with you. Starting the next full cycle after MatB is a new full entry from your accumulated funds; if they come up short, you are parked until the gap is covered (see Rescue).
...
## Compensation Plan (verified from contract, V8.52 PRIVATE)
...
Epoch advances when the FIRST fires: 1,000,000 CNOVA minted, 1,000 new members, or 180 days elapsed (V8.52 PRIVATE policy — item 42).
...
On the 25TH of every month a distribution can be triggered (calendar-day model, V8.52 PRIVATE): half the pool pays out, half rolls over and compounds. Members then have 30 days to claim; unclaimed amounts sweep back into the pool by design. Payouts begin at mainnet (policy).
...
## Upgrades, Re-entry & Rescue (verified from contract, V8.52 PRIVATE)
...
- When the newest pair FILLS, the factory spawns the next pair and overflow routes into it automatically (V8.52 PRIVATE — the old fixed 375/381 entry thresholds are GONE from the contracts; think seats remaining, not entry counts).
- <b>"Why is T1.2 (or any new pair) empty?"</b> New pairs deploy when the newest pair is FULL (V8.52 PRIVATE item 33 — no more fixed entry thresholds). A new pair can sit empty until routing reaches it — only then does new activity flow into it automatically. An empty freshly-deployed pair is the system working correctly: the runway is built before the plane needs it. It is NOT stalled, and nobody is "stuck" in it — reassure members this is by design (V8.52 PRIVATE anti-freeze buffer).
...
## Contracts (Base Sepolia — V8.52 PRIVATE)
TierRouter: <code>0x72DAF5647Dff8E71D0c4ECB768B19E77DcBcba63</code>
MatrixKeeper: <code>0xe5Cc6b6A4D780e6B12606940b55dDA5AF18412FB</code>
CNOVA Token: <code>0x0DB48e7Aa5a972627bFA2f69f518328c053B0832</code>
  ...
  if (isCommand) {
    ...
    if (cmd === '/tier') {
      await sendReply(BOT_TOKEN, chatId,
        ...
        `• 2.5% instant earn on join · 50% crossing reserve (pre-pays your crossing in full — V8.52 PRIVATE)\n` +
