"""
update_addrs_v8_45.py — Replace all V8.43 contract addresses with V8.45 in frontend HTML files.
Run from CryptoNova-Testnet-App: python update_addrs_v8_45.py
...
OLD_JSON = os.path.join(CONTRACTS_DIR, "scripts", "deployed_addresses_v8_44.json")
NEW_JSON = os.path.join(CONTRACTS_DIR, "scripts", "deployed_addresses_v8_45.json")
...
REPLACEMENTS.append((
    ...
    "V8.45 — deployed 2026-07-26 — V8.44 + nested-entry BFS integrity fix (no phantom seats, self-healing rotation)"
))
# Version tag used in console logs / comments
REPLACEMENTS.append(("[v8.44]", "[v8.45]"))
REPLACEMENTS.append(("v8.44", "v8.45"))
...
print("  2. git add -A && git commit -m 'chore(V8.45): update all contract addresses'")
print("  3. git push origin admin   (preview/main stay on V8.43 until cutover)")
print("  4. V8.45 FRONTEND NOTES: withdrawableOf now INCLUDES pending pool accrual;")
