"""
update_addrs_v8_44.py — Replace all V8.43 contract addresses with V8.44 in frontend HTML files.
Run from CryptoNova-Testnet-App: python update_addrs_v8_44.py
...
OLD_JSON = os.path.join(CONTRACTS_DIR, "scripts", "deployed_addresses_v8_43.json")
NEW_JSON = os.path.join(CONTRACTS_DIR, "scripts", "deployed_addresses_v8_44.json")
...
REPLACEMENTS.append((
    ...
    "V8.44 — deployed 2026-07-25 — contract-driven cycling: reserve-funded re-entry, park-not-exit, own-pair overflow, pull-based pool, exitSeat"
))
# Version tag used in console logs / comments
REPLACEMENTS.append(("[v8.43]", "[v8.44]"))
REPLACEMENTS.append(("v8.43", "v8.44"))
...
print("  2. git add -A && git commit -m 'chore(V8.44): update all contract addresses'")
print("  3. git push origin admin   (preview/main stay on V8.43 until cutover)")
print("  4. V8.44 FRONTEND NOTES: withdrawableOf now INCLUDES pending pool accrual;")
print("     PoolShareCredited fires at settle (aggregated), not per rotation;")
print("     new fns: registerWithOptions/Permit, bulkWithdraw, hybridUpgrade, exitSeat (20% reserve penalty).")
